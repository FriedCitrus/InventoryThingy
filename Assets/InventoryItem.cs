...
public class InventoryItem : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public Item item; // Reference to the Item scriptable object containing item data
    ...
    {
        item = newItem; // Assign the new item to the inventory item
        image.sprite = item.itemIcon; // Set the item's icon in the Image component
    ...
    public void UpdateItemCount()
    {
        ...
        bool textActive = item.isStackable && itemCount > 1; // Show count text only if the item is stackable and count is greater than 1
